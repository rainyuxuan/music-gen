...
class Trainer:
    ...
    def train(self,
              ...
              learning_rate: float,
              num_epochs: int,
              device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")) -> pd.DataFrame:
        ...
        return pd.DataFrame(history, columns=['train_loss', 'val_loss', 'train_acc', 'val_acc'])
